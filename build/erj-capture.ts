...
interface ErjDoor { label: string; href: string; }
interface ErjConfig {
  whatsapp: string;
  whatsappDisplay: string;
  channel: string;
  capacity: {
    placementTotal: number; placementTaken: number;
    innerCircleTotal: number; innerCircleTaken: number;
  };
  evergreen: { lead: string; body: string; doors: ErjDoor[] };
  messages: Record<string, string>;
}

interface Window {
  ERJ_CONFIG?: ErjConfig;
  ERJ_NAV?: { base?: string;[k: string]: unknown };
  ERJ_CAPTURE?: { waLink: (t: string, v?: Record<string, string>) => string; refresh: () => void };
}
...
(function () {
  ...
  function waLink(templateKey: string, vars?: Record<string, string>): string {
...
`;
  ...
  function injectCss(): void {
    if (document.getElementById('erj-capture-css')) return;
  ...
  function evergreenDoors(): void {
    const panels = Array.from(document.querySelectorAll<HTMLElement>('[data-deadline]'));
    if (!panels.length) return;

    const seen = new Set<Element>();

    panels.forEach(panel => {
      if (panel.hasAttribute('data-no-evergreen')) return;
      const host = panel.closest('section, .timer-card, .cd-card, div') || panel;
      if (seen.has(host) || host.querySelector('.evergreen')) return;
      seen.add(host);
  ...
  function capacityLines(): void {
    const nodes = Array.from(document.querySelectorAll<HTMLElement>('[data-erj-capacity]'));
  ...
  function readingCta(): void {
    const nodes = Array.from(document.querySelectorAll<HTMLElement>('[data-erj-cta]'));
    nodes.forEach(node => {
      if (node.querySelector('.cap-read')) return;
  ...
  function boot(): void {
    try { injectCss(); } catch (e) { /* never break the page */ }
    try { scanCapture(); } catch (e) { }
    try { channelBridge(); } catch (e) { }
    try { evergreenDoors(); } catch (e) { }
    try { capacityLines(); } catch (e) { }
    try { readingCta(); } catch (e) { }
  }
