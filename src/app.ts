...
interface Window { ERJ?: Record<string, unknown>; }
...
(function () {
  ...
  const $ = <T extends Element = Element>(s: string, r: ParentNode = document) =>
    r.querySelector<T>(s);
  const $$ = <T extends Element = Element>(s: string, r: ParentNode = document) =>
    Array.prototype.slice.call(r.querySelectorAll<T>(s)) as T[];
  ...
  function theme(): void {
    ...
    const apply = (mode: string) => {
      ...
      if (meta) meta.setAttribute('content', mode === 'day' ? '#FAFAF8' : '#000000');
      $$('[data-theme-toggle]').forEach(b => {
        b.setAttribute('aria-label',
          mode === 'day' ? 'Switch to dark theme' : 'Switch to light theme');
  ...
  function drawer(): void {
    const panel = $<HTMLElement>('[data-drawer]');
    const scrim = $<HTMLElement>('[data-scrim]');
    const btn = $<HTMLElement>('[data-drawer-open]');
    if (!panel || !scrim || !btn) return;

    let last: HTMLElement | null = null;

    const focusables = () =>
      $$<HTMLElement>('a[href],button:not([disabled])', panel);
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        if (el.hasAttribute('data-reveal-stagger')) {
          $$<HTMLElement>(':scope > *', el).forEach((child, i) => {
  ...
  function images(): void {
    $$<HTMLImageElement>('.media img').forEach(img => {
      ...
      img.addEventListener('error', () => {
        ...
        if (holder) (holder as HTMLElement).style.background = 'var(--surface-2)';
      }, { once: true });
    });
  }
  ...
  function scrollUi(): void {
    const hdr = $<HTMLElement>('[data-header]');
    const bar = $<HTMLElement>('[data-progress]');
    const top = $<HTMLElement>('[data-totop]');
    let ticking = false;
    ...
    const run = () => {
      ...
      if (top) {
        if (y > innerHeight * 0.9) top.setAttribute('data-show', '');
  ...
  function counters(): void {
    const nodes = $$<HTMLElement>('[data-count]');
  ...
  function boot(): void {
    units.forEach(([name, fn]) => {
      try { fn(); } catch (err) {
        if (console && console.warn) console.warn('[erj] ' + name + ' failed', err);
      }
    });
  }
