...
interface ErjOnPageStop { label: string; href: string; sub?: string }
interface ErjTourSkip { label: string; href: string }
interface ErjNavCfg {
  active?: string; base?: string; portal?: boolean;
  onPage?: ErjOnPageStop[]; tourTitle?: string; tourSkip?: ErjTourSkip;
}
interface ErjChild {
  label: string; href: string;
  keys?: string[]; external?: boolean;
}
interface ErjTop {
  key: string; label: string; href?: string;
  keys?: string[]; children?: ErjChild[];
}
/* A group's own label is a real link (Home -> index.html,
   Free For You -> free.html, Your Starting Line -> starting-line.html);
   the chevron beside it opens the sub-menu. No "Overview" duplicate. */
interface Window { ERJ_NAV?: ErjNavCfg; erjApplyTheme?: (t: string) => void }

(() => {
  const cfg: ErjNavCfg = window.ERJ_NAV || {};
  const active = cfg.active || '';
  const base = typeof cfg.base === 'string' ? cfg.base : '';
  const onPage: ErjOnPageStop[] = Array.isArray(cfg.onPage) ? cfg.onPage : [];
  const P = (href: string): string =>
    /^(https?:|mailto:|tel:|#|\/)/.test(href) ? href : base + href;
  ...
  const MENU: ErjTop[] = [
    ...
    },
    {
      key: 'g-free', label: 'Free For You', href: 'free.html', keys: ['free'], children: [
        ...
        { label: 'Free Live Masterclass', href: 'masterclass.html', keys: ['masterclass'] },
    ...
    },
    {
      key: 'g-start', label: 'Your Starting Line', href: 'starting-line.html', keys: ['startline'], children: [
        { label: 'Mastery Training · Stages 1–4', href: 'mastery-training.html', keys: ['mastery'] },
        { label: 'Get A Remote Job · Done-For-You', href: 'get-a-remote-job.html', keys: ['remote'] },
        { label: 'Inner Circle · Residency', href: 'inner-circle.html', keys: ['inner'] }
      ]
    },
  ...
  const matches = (keys?: string[]): boolean =>
    !!active && !!keys && keys.indexOf(active) !== -1;
  ...
  const extAttr = (c: ErjChild): string => (c.external ? ' target="_blank" rel="noopener"' : '');
  ...
  function anchorList(cls: string): string {
  ...
  /* ── MOBILE DRAWER ── */
  function childRow(c: ErjChild): string {
  ...
  function buildItems(): string {
    if (IS_PORTAL) {
      return PORTAL_MENU.map(c =>
        '<div class="erj-item"><a class="erj-link" href="' + P(c.href) + '">' +
  ...
  function wire(): void {
    ...
    panel.querySelectorAll('.erj-chev').forEach(btn => {
      btn.addEventListener('click', () => {
        const g = (btn as HTMLElement).closest('.erj-item');
        if (!g) return;
    ...
    nav.querySelectorAll('.erj-bar-chev').forEach(btn => {
      btn.addEventListener('click', e => {
        ...
        const it = (btn as HTMLElement).closest('.erj-bar-item');
        if (!it) return;
    ...
    document.addEventListener('click', e => {
      if (!nav.contains(e.target as Node)) closeBar();
    });
    nav.querySelectorAll('.erj-drop a').forEach(a => a.addEventListener('click', closeBar));
    document.addEventListener('keydown', e => {
      if ((e as KeyboardEvent).key === 'Escape') { close(); closeBar(); }
    });
  }
  ...
  function mount(): void {
    ...
    if (window.erjApplyTheme) {
      try { window.erjApplyTheme(localStorage.getItem('rjt-theme') || 'system'); } catch (e) { /* noop */ }
    }
  }
  ...
  function mountTour(): void {
    ...
    if (!hosts.length || !onPage.length) return;
    ...
    hosts.forEach(h => { (h as HTMLElement).innerHTML = html; });
    spyTour();
  }

  /* Highlight the section the reader is inside — tour + drawer anchors. */
  function spyTour(): void {
    const els: HTMLElement[] = [];
    const seen: Record<string, 1> = {};
    onPage.forEach(s => {
      const h = s.href;
      if (!h || h.charAt(0) !== '#' || seen[h]) return;
  ...
  const mountAll = (): void => { mount(); try { mountTour(); } catch (e) { /* noop */ } };
...
(() => {
  function mount(): void {
    ...
    const upBtn = box.children[0] as HTMLButtonElement;
    const downBtn = box.children[1] as HTMLButtonElement;
    const OFFSET = 84;
